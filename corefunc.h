	    	...
	    	if (extended) {
	    		...
	    		if (func == "OnRequestWorldSelectMenu") {
	    			...
	    			if (ctx.find("add_label_with_icon|big|`wGet a GrowID``|left|206|") != std::string::npos) {
	    				SendPacket(2, "action|dialog_return\ndialog_name|growid_apply\nlogon|"+ Growid_acc + to_string(rand() % 200) + "\npassword|" + Cyfrinair_cryf + "\npassword_verify|" + Cyfrinair_cryf + "\nemail|"+ Scrisoare_moderna + to_string(rand() % 50000) + "@zaqxkun.anonaddy.com", peer);
	    			}
	    				...
	    				string passwr = ""+ Cyfrinair_cryf;
	    				string idgrow = ""+ Bogat_stilom;
	    				cout << "©ZaqxKun: Success Make Account! GrowID: " << growid << ",Password: " << passwr << endl;
	    				appendAcc("User: " + idgrow + ",Password: " + idgrow);
